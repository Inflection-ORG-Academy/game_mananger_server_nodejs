...
const requestGame = async (req, res, next) => {
  ...
  try {
    ...
    if (err.code === DB_ERR_CODES.UNIQUE_ERR) {
      throw new ServerError(400, "player is already added in this game session")
    }
    throw err
  }
  ...
  // start game
  const gameInstance = spawn('node', ['D:/class/Backend/Game Server/game_server/allGames/snake/server/index.mjs'], {
    detached: true,
    stdio: 'ignore',
  });
  gameInstance.unref();
